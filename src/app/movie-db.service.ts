...
export class MovieDBService {
  ...
  getByKeywordIDChrono() {
    return this.http.get(`https://api.themoviedb.org/3/discover/movie?api_key=${key}&sort_by=primary_release_date.asc&with_keywords=${mcuKeywordID}&without_genres=${documentaryID},${tvMovieID}&without_keywords=${shortID}&with_runtime.gte=60&release_date.lte=${new Date().toJSON().slice(0,10)}`)
  }
  getByKeywordIDChronoNextPage(page) {
    return this.http.get(`https://api.themoviedb.org/3/discover/movie?api_key=${key}&sort_by=primary_release_date.asc&page=${page}&with_keywords=${mcuKeywordID}&without_genres=${documentaryID},${tvMovieID}&without_keywords=${shortID}&with_runtime.gte=60&release_date.lte=${new Date().toJSON().slice(0,10)}`)
  }
